                ...
                return new Promise((resolve, reject) => {
                    email.find({})
                        .then((data) => {
                            var id = []
                            _.map(data, (val, key) => {
                                if ((val.subject.match(new RegExp(tag.title, 'gi'))) || ((tag.to < 0 && tag.from < 0) && (new Date(val.date).getTime() < new Date(tag.to).getTime() && new Date(val.date).getTime() > new Date(tag.from).getTime())) || (val.sender_mail.match(new RegExp(tag.email, 'gi')))) {
                                    id.push(val._id);

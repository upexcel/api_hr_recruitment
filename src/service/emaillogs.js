            ...
            if (data && data.status) {
                if (log.emailLogs) {
                    ...
                    if (!data.user)
                        ...
                        logs = new log({ email: data.email_response.envelope.to, from: data.email_response.envelope.from, time: new Date(), user: data.user, subject: data.subject, body: data.body })
                }

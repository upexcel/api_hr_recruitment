    ...
    emailLog(log, data) {
        return new Promise((resolve, reject) => {
            ...
            if (data && data.status) {
                ...
                if (log.emailLogs) {
                    logs = new log.emailLogs({ email: data.email_response.envelope.to, from: data.email_response.envelope.from, time: new Date(time), user: log.user.email, subject: data.subject, body: data.body })
                } else {

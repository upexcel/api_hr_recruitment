...
export class FetchController extends BaseAPIController {
    ...
    fetch = (req, res, next) => {
        ...
        req.email.find(where, { "date": 1, "email_date": 1, "from": 1, "sender_mail": 1, "subject": 1, "unread": 1, "attachment": 1, "tag_id": 1 }).sort({ date: -1 }).skip((page - 1) * parseInt(limit)).limit(parseInt(limit)).exec((err, data) => {
            if (err) {
                next(err);
            } else {
                if (data[0] == null) {
